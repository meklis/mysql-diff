...
class Schema {
    ...
    {
        $this->dbName = $dbName;
        $this->sql = new \mysqli($host, $username, $passwd, $dbName, $port);
        if($this->sql->connect_error) {
            throw new \Exception("Error connect to $host/$dbName: ". $this->sql->connect_error );
        }
    }
    ...
    public function getConstraintFKs() {
        $data = $this->sql->query("select 
       c.constraint_name constraint_name, 
       c.table_name table_name, 
       c.column_name column_name, 
       c.referenced_table_name referenced_table_name, 
       c.referenced_column_name referenced_column_name, 
    ...
    protected function getTablesView($type) {
        $tables = [];
        $data = $this->sql->query("SELECT table_name table_name,
       table_type table_type, 
       `engine` engine 
        FROM information_schema.`TABLES` WHERE TABLE_SCHEMA = '{$this->dbName}' and table_type like '%$type%'");
        while ($d = $data->fetch_assoc()) {
            $collumns = [];
